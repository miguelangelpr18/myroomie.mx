  ...
  const fetchZones = useCallback(async (q: string, locId: string) => {
    ...
    try {
      ...
      if (res.status === 500 || res.status === 401 || res.status === 403) {
        setFetchError('Error de conexión con el mapa, revisa tu configuración')
        setZones([])
        setLoading(false)
        return
      }
      ...
      console.error('[ZoneAutocomplete] Error:', err)
      setFetchError('Error de conexión con el mapa, revisa tu configuración')
      setZones([])
    }
    setLoading(false)
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter') {
      ...
      if (open && zones.length > 0) {
        handleSelect(zones[0]) // Seleccionar primera opción si hay resultados
      }
    }
  }
