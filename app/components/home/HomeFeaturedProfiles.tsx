...
export default async function HomeFeaturedProfiles() {
  ...
  let profiles = featured
  let isFallback = false

  // 2. Fallback: si no hay destacados, traer recientes
        ...
        <div className="flex justify-between items-end gap-4 mb-6">
          <div>
            <h2 className="text-lg md:text-xl font-semibold tracking-[-0.01em] text-neutral-900">
              {isFallback ? 'Roomies recientes' : 'Roomies destacados'}
            </h2>
            <p className="text-sm text-neutral-600 mt-1">
              {isFallback ? 'Personas buscando con quién compartir' : 'Perfiles promovidos para mayor visibilidad'}
            </p>
          </div>
          <Link
            href="/explore"
