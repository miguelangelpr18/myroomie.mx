...
export default function LocationPickerField({
      ...
      try {
        ...
        if (hasLatLng) {
          try {
            ...
            // reverse falló: zone queda '', city ya tiene fallback
          }
        }

        setSelected({ location_id: locationId, label: candidate.label, city, zone })
        setResults([])
        setQuery(candidate.label)
        onSelect({ location_id: locationId, city, zone })
      } catch {
        setFetchError('No se pudo guardar la ubicación')
      }
      setLoading(false)
  ...
  return (
    <div ref={containerRef} className="relative">
      <label htmlFor="location-picker" className="block mb-2 font-medium text-neutral-700">
      ...
      <input
        ...
        `}
        aria-label="Ubicación (buscar ciudad o colonia)"
        aria-describedby={displayError ? 'location-picker-error' : undefined}
      ...
      {displayError && (
        <p id="location-picker-error" className="mt-1.5 text-sm text-red-600">
          {displayError}
        </p>
      )}
      {selected && (
