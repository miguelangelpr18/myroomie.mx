...
- The `listings` table was previously created directly in the Supabase dashboard with no tracked file. Migration 003 reconstructs it from the `squema.sql` column snapshot and confirmed app code behavior.
- `is_active` is used in the app but was absent from `squema.sql` — added in migration 011. **Verify this column exists in production before deploying migration 011.**
